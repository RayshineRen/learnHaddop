...
def demo_lazy_execution(sc: MiniSparkContext) -> None:
    driver = sc.driver
...
def demo_shuffle_and_stage(sc: MiniSparkContext) -> None:
    driver = sc.driver
    data = ["a", "b", "a", "c", "b", "a"]
...
def demo_lineage_recovery(sc: MiniSparkContext) -> None:
    driver = sc.driver
...
def demo_dataframe(sc: MiniSparkContext) -> None:
    driver = sc.driver
...
def main() -> None:
    ...
    demo_lazy_execution(sc)
    demo_shuffle_and_stage(sc)
    demo_lineage_recovery(sc)
    demo_dataframe(sc)
